...
[issue #1847](https://github.com/Live-Charts/LiveCharts2/issues/1847)).
...
- **Treat as missing.** Map to `Coordinate.Empty`. The point is invisible and
  not tooltipable, like a `null` entry.
- **Keep visible with a custom tooltip.** Map to a sentinel like
  `new Coordinate(index, 0)` so the bar collapses to a stub but the column's
  ...
  still see what was at that index on hover.

Pick whichever fits your domain - both are valid overrides of the default
pass-through behavior.

### Custom mapper (raw `double` / `float` values)

For ad-hoc series, scope the mapper via the `Mapping` property:
...
`IChartEntity` types compute their own `Coordinate` and never go through the
